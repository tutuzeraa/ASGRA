...
def _is_in_split(img_key, split):
    ...
    return (
        (split == "train" and key <= 1000 ) or
        (split == "val" and key > 1000 and key <= 1200) or
        (split == "test" and key > 1200)
    )
...
class RCPDSceneGraphDataset(Dataset):
    def __init__(self, data_dir, split, cache_file = None):
        ...
        for fp in self.files:
            with fp.open() as fh:
                for line in fh: 
                    js = json.loads(line)
                    key = js["image_id"]["image_id"]
                    if key not in rcpd_csam_labels:
                        continue
                    if rcpd_csam_split.get(key) != self.split:
                        continue
                    is_csam = rcpd_csam_labels[key]
                    self.data.append(self._scan_line(js, is_csam, js["image_id"]["image_id"]))

        super().__init__(root=None)

        logger.info("Finished ✅. For split %s: Loaded %d graphs, %d object tokens, %d relations",
                    self.split, len(self.data), len(self.word2idx), len(self.rel2idx))
    ...
    def _scan_line(self, js: dict, is_csam: bool, img_key: str) -> Data:
        """
